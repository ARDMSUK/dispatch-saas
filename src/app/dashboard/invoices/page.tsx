    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <div>
                    <h1 className="text-3xl font-bold tracking-tight text-slate-900 mb-2">Invoices</h1>
                    <p className="text-slate-500">Manage corporate billing, generate statements, and track account payments.</p>
                </div>

                <Dialog open={isGeneratorOpen} onOpenChange={setIsGeneratorOpen}>
                    <DialogTrigger asChild>
                        <Button className="bg-indigo-600 hover:bg-indigo-500 text-slate-900 shadow-lg">
                    ...
                    <DialogContent className="sm:max-w-[700px] bg-white border-slate-200">
                        ...
                        <div className="space-y-4 py-4">
                            <div className="space-y-2">
                                <label className="text-sm font-medium text-slate-600">Select Corporate Account</label>
                            ...
                            {generatorAccount && (
                                <div className="space-y-2 pt-2">
                                    <div className="flex justify-between items-end">
                                        <label className="text-sm font-medium text-slate-600">Unbilled Completed Jobs ({unbilledJobs.length})</label>
                                        <Button
                                            variant="ghost"
                                            size="sm"
                                            className="h-6 text-xs text-indigo-600 hover:text-indigo-300"
                                    ...
                                    <div className="border border-slate-200 rounded-md bg-slate-100 max-h-[300px] overflow-y-auto p-2">
                                        {unbilledJobs.length === 0 ? (
                                            <p className="text-sm text-slate-400 text-center py-4">No unbilled jobs found for this account.</p>
                                        ) : (
                                            <div className="space-y-1">
                                                {unbilledJobs.map(job => (
                                                    <div
                                                        key={job.id}
                                                        className={`flex items-center justify-between p-2 rounded cursor-pointer transition-colors border ${selectedJobs.includes(job.id) ? 'bg-indigo-900/20 border-indigo-500/50' : 'bg-white border-transparent hover:bg-slate-200/50'}`}
                                                    ...
                                                    >
                                                        <div className="flex items-center gap-3">
                                                            ...
                                                            <div className="flex flex-col">
                                                                <span className="text-sm font-medium text-slate-800">TRIP-{job.id}</span>
                                                                <span className="text-xs text-slate-400">{format(new Date(job.pickupTime), 'MMM do')} • {job.passengerName}</span>
                                                            </div>
                                                        </div>
                                                        <span className="text-sm font-mono text-slate-600">£{job.fare?.toFixed(2)}</span>
                                                    </div>
                                                ))}
                                            </div>
                                        )}
                                    ...
                                    {selectedJobs.length > 0 && (
                                        <div className="bg-slate-100 p-3 rounded-md flex justify-between items-center mt-2 border border-slate-200">
                                            <div className="flex items-center gap-2 text-slate-500">
                                                <Calculator className="w-4 h-4" />
                                                <span className="text-sm">Subtotal ({selectedJobs.length} trips)</span>
                                            </div>
                                            <span className="text-lg font-bold text-slate-900">£{calcGeneratorSubtotal.toFixed(2)}</span>
                                        </div>
                                    )}
            ...
            <Card className="bg-white border-slate-200">
                <div className="overflow-x-auto">
                    <Table>
                        <TableHeader>
                            <TableRow className="border-slate-200 bg-slate-100 hover:bg-zinc-900/50">
                                <TableHead className="text-slate-500">Invoice ID</TableHead>
                                <TableHead className="text-slate-500">Account</TableHead>
                                <TableHead className="text-slate-500">Issued</TableHead>
                        ...
                        <TableBody>
                            {loading ? (
                                <TableRow>
                                    <TableCell colSpan={7} className="h-32 text-center text-slate-400">Loading invoices...</TableCell>
                                </TableRow>
                            ) : invoices.length === 0 ? (
                                <TableRow>
                                ...
                                invoices.map(inv => (
                                    <TableRow key={inv.id} className="border-zinc-800/50 hover:bg-zinc-900/50">
                                        <TableCell className="font-mono text-slate-800">{inv.invoiceNumber}</TableCell>
                                        <TableCell>
                                            <div className="flex flex-col">
                                                <span className="font-medium text-slate-800">{inv.account?.name || 'Unknown'}</span>
                                                <span className="text-xs text-slate-400">{inv.account?.code}</span>
                                            </div>
                                        </TableCell>
                                        <TableCell className="text-slate-500">{format(new Date(inv.issueDate), 'MMM do, yyyy')}</TableCell>
                                        <TableCell className="text-slate-500">{format(new Date(inv.dueDate), 'MMM do, yyyy')}</TableCell>
                                        <TableCell className="text-right font-bold text-slate-900">£{inv.total.toFixed(2)}</TableCell>
                                        <TableCell className="text-center">
                                            {inv.status === 'PAID' ? (
                                                <Badge className="bg-emerald-500/10 text-emerald-600 hover:bg-emerald-500/20">PAID</Badge>
                                            ) : inv.status === 'OVERDUE' ? (
                                                <Badge className="bg-red-500/10 text-red-400 hover:bg-red-500/20">OVERDUE</Badge>
                                            ) : (
                                                <Badge className="bg-blue-700/10 text-blue-700 hover:bg-blue-700/20">{inv.status}</Badge>
                                            )}
                                        </TableCell>
                                        <TableCell>
