                ...
                <div className="flex-1 relative z-10 flex flex-col overflow-hidden">
                    ...
                    <div className="flex items-center justify-between p-2 border-b border-slate-200 bg-white shrink-0">
                        <div className="flex gap-2">
                            <Button variant="default" className="bg-amber-400 hover:bg-amber-500 text-xs font-bold shadow-sm h-8 px-4 rounded-full">PENDING (5)</Button>
                            ...
                            <Button variant="secondary" className="bg-blue-100 hover:bg-blue-200 text-amber-600 text-xs font-bold h-8 px-4 rounded-full">DISPATCHED (2)</Button>
                        </div>
                            ...
                            {MOCK_JOBS.map((job, idx) => (
                                <div key={idx} className="flex items-center gap-3 bg-white border border-slate-200 p-2 rounded-lg shadow-sm hover:border-blue-400 cursor-pointer transition-all group">
                                    ...
                                    <div className="w-16 flex flex-col justify-center border-r border-slate-100 pr-2 h-full">
                                        <div className="font-black text-amber-400 text-base leading-none">{job.time}</div>
                                    ...
                                    <div className="w-20 flex flex-col items-center justify-center gap-1.5 border-r border-slate-100 pr-2 h-full">
                                        <span className={`font-bold text-[9px] px-1.5 py-0.5 rounded w-full text-center ${job.status === 'PENDING' || job.status === 'UNASSIGNED' ? 'bg-blue-100 text-amber-500' : 'bg-emerald-100 text-emerald-700'}`}>{job.status}</span>
                                        {job.driver ? (
                                    ...
                                    <div className="flex-1 min-w-0 flex flex-col justify-center gap-1.5 border-r border-slate-100 pr-2">
                                        ...
                                        {(job.notes || job.flight || job.badges.length > 0) && (
                                            <div className="flex items-center gap-2 text-[10px] mt-0.5">
                                                ...
                                                {job.flight && <span className="bg-blue-50 text-amber-400 px-1.5 py-0.5 border border-blue-200 rounded font-mono flex items-center gap-1">✈️ {job.flight}</span>}
