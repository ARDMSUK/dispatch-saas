...
export async function POST(
    ...
    try {
        ...
        if (!job.passengerEmail && !job.customer?.email) {
            return NextResponse.json({ error: 'Customer email address is required to send payment link email' }, { status: 400, headers: corsHeaders });
        }
        ...
        return NextResponse.json({ success: true, emailResult, reused: !!job.paymentLink && job.paymentLink === paymentLink }, { headers: corsHeaders });
        ...
        console.error("POST /api/mobile/driver/jobs/[id]/payment/email error:", redactedMessage);
