        ...
        <Card className="h-full bg-slate-100 border-slate-200 flex flex-col">
            <CardHeader className="pb-3 border-b border-slate-200">
                <CardTitle className="text-slate-900 text-lg flex items-center justify-between">
                    Live Queue
                    <Badge variant="secondary" className="bg-indigo-600/10 text-indigo-600 hover:bg-indigo-600/20">
                        {jobs.length} Pending
                    </Badge>
                </CardTitle>
            </CardHeader>
            <CardContent className="flex-1 overflow-y-auto p-0 scrollbar-thin scrollbar-thumb-zinc-700">
                {loading && jobs.length === 0 ? (
                    <div className="p-4 text-slate-400 text-sm text-center">Loading feed...</div>
                ) : jobs.length === 0 ? (
                    <div className="p-4 text-slate-400 text-sm text-center">No pending jobs</div>
                ) : (
                    <div className="divide-y divide-white/5">
                        {jobs.map(job => (
                            <div
                                ...
                                className={`p-4 cursor-pointer transition-colors hover:bg-slate-200 ${selectedJobId === job.id ? 'bg-indigo-600/10 border-l-2 border-indigo-600' : 'border-l-2 border-transparent'}`}
                            >
                                <div className="flex justify-between items-start mb-2">
                                    ...
                                    <Badge variant="outline" className="border-slate-300 text-slate-500 text-[10px] h-5">
                                        {job.vehicleType}
                                    </Badge>
                                </div>
                                ...
                                <div className="space-y-2 mb-3">
                                    <div className="flex gap-2">
                                        ...
                                        <div className="flex-1 space-y-2 min-w-0">
                                            ...
                                            <p className="text-sm text-slate-500 truncate" title={job.dropoffAddress}>
