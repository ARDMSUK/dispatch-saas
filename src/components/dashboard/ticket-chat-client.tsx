    ...
    const getStatusBadge = () => {
        switch (status) {
            case 'PENDING_AI_REVIEW':
                return <Badge variant="outline" className="text-amber-400 border-blue-400/20 bg-blue-400/10"><Clock className="w-3 h-3 mr-1" /> AI Active</Badge>;
            case 'ANSWERED':
                        ...
                        {messages.map((m) => (
                            <div key={m.id} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                                <div className={`flex max-w-[80%] ${m.role === 'user' ? 'flex-row-reverse' : 'flex-row'} items-end gap-2`}>
                                    <div className={`h-8 w-8 rounded-full flex items-center justify-center shrink-0 ${m.role === 'user' ? 'bg-amber-500 text-black' : 'bg-amber-400 text-slate-900'}`}>
